...
program:
    T_PROGRAM T_ID declarations T_BEGIN statements T_END
    {
        ...
        for(std::pair<std::string,var_data> v : symbol_table)
        {
            ...
            if(v.second.decl_type == int_arr)
                std::cout << v.second.label << ": resd " << v.second.size << std::endl;
...
|
    T_INTEGER T_ARRAY T_ID T_SOPEN T_NUM T_SCLOSE T_SEMICOLON
    {
        ...
        symbol_table[*$3] = var_data( d_loc__.first_line, int_arr, new_label(), *$5 );
...
assignment:
    ...
    T_ID T_SOPEN expression T_SCLOSE T_ASSIGN expression T_SEMICOLON
    {
        ...
        if(symbol_table[*$1].decl_type != int_arr || $3->expr_type != integer || $6->expr_type != integer)
        {
           std::stringstream ss;
        ...
        $$ = new std::string("" +
                ...
                $6->expr_code +
                "pop ebx\n" +
                "mov [" + symbol_table[*$1].label + "+4*ebx], eax\n");
...
read:
    ...
    T_READ T_OPEN T_ID T_SOPEN expression T_SCLOSE T_CLOSE T_SEMICOLON
    {
        ...
        if(symbol_table[*$3].decl_type != int_arr || $5->expr_type != integer)
        {
           std::stringstream ss;
           ss << d_loc__.first_line << ": Type error." << std::endl;
           error( ss.str().c_str() );
        ...
        $$ = new std::string("" +
                ...
                "call read_unsigned\n" +
                "pop ebx\n" +
                "mov [" + symbol_table[*$3].label + "+4*ebx], eax\n");
...
|
    T_ID T_SOPEN expression T_SCLOSE
    {
        ...
        if(symbol_table[*$1].decl_type != int_arr || $3->expr_type != integer)
        {
           std::stringstream ss;
           ss << d_loc__.first_line << ": Type error." << std::endl;
           error( ss.str().c_str() );
        ...
        $$ = new expression_descriptor(integer, "" +
                $3->expr_code +
                "mov eax, [" + symbol_table[*$1].label + "+4*eax]\n");
...
|
    expression T_EQ expression
    {
        ...
        if($1->expr_type != $3->expr_type || $1->expr_type == int_arr)
        {
           std::stringstream ss;
           ss << d_loc__.first_line << ": Type error." << std::endl;
           error( ss.str().c_str() );
        ...
        /*
        * Added check for int_arr since we cannot decide
